import {
  APP_VERSION, CONFIG, ROLE, SESSION_STATUS, QUESTION_BANK_VERSION, CONSENT_VERSION,
...
import { PROTOCOLS, SET_ORDERS, buildTimeline, getPhaseAtElapsed } from './protocols.js';
import {
...
const PROTOCOL_LABELS={RESEARCH_V1:'研究モード (RESEARCH_V1)',EVENT_V1:'イベントモード (EVENT_V1)'};
...
function sessionLobby(){
  ...
  return shell(`<div class="layout lobby-layout"><main class="panel panel-strong">
    ...
    <div class="lobby-columns"><section>
      <h3>新しい計測</h3>
      <div class="field"><label>Protocol</label><select id="protocol"><option value="RESEARCH_V1">RESEARCH_V1 — 研究</option><option value="EVENT_V1">EVENT_V1 — 体験</option></select></div>
      <div class="field"><label>Question Category</label><select id="category">${Object.keys(QUESTION_BANK).map(x=>`<option>${esc(x)}</option>`).join('')}</select><small>質問カテゴリと、個別アンケートで回答する関係性は別データとして保存します。</small></div>
...
async function createSession(){
  ...
  try{
    const protocolId=document.querySelector('#protocol')?.value||'RESEARCH_V1';const category=document.querySelector('#category')?.value||'友達';
    ...
    const setOrder=await chooseLeastUsedOrder(protocolId,category,pa,pb);const timeline=buildTimeline(protocolId,setOrder,QUESTION_BANK[category]);
    ...
    const s={
      ...
      timeline,createdAt:now(),appVersion:APP_VERSION,questionBankVersion:QUESTION_BANK_VERSION,consentVersion:CONSENT_VERSION,preprocessingVersion:PREPROCESSING_VERSION,
      ...
      sensorReady:{A:false,B:false},surveyStatus:{preA:false,preB:false,postA:false,postB:false},devicePresence:{[state.role]:now()},guardianOrSchoolConsent:'externally_managed'
    };
...
function operatorScreen(){
  ...
  const postButton=s.stage==='POST_SURVEY'?`<button class="btn full ${s.surveyStatus?.[`post${side}`]?'good':''}" data-action="open-post-survey" ${s.surveyStatus?.[`post${side}`]?'disabled':''}>${s.surveyStatus?.[`post${side}`]?'事後アンケート 回答済み':`${side} 事後アンケート`}</button>`:'';
  ...
  <div class="layout operator-layout">
    ...
    <aside class="panel control-panel">
      ...
      ${readinessRow('A 事前アンケート',s.protocolId==='EVENT_V1'||!!s.surveyStatus?.preA,s.protocolId==='EVENT_V1'?'EVENTでは不要':'個別・非公開')}
      ${readinessRow('B 事前アンケート',s.protocolId==='EVENT_V1'||!!s.surveyStatus?.preB,s.protocolId==='EVENT_V1'?'EVENTでは不要':'個別・非公開')}
      ...
      ${postButton}
      ${s.stage==='POST_SURVEY'&&isController()&&s.surveyStatus?.postA&&s.surveyStatus?.postB?'<button class="btn primary full" data-action="calculate">結果を計算</button>':''}
...
async function startAcclim(){
  ...
  if(s.protocolId==='EVENT_V1'){await backend.updateSession(s.sessionId,{stage:'READY',status:SESSION_STATUS.READY});await backend.appendAuditLog(s.sessionId,{eventType:'EVENT_READY'});return;}
...
async function finishMeasurement(){
  ...
  try{const next=state.session.protocolId==='RESEARCH_V1'?'POST_SURVEY':'CALCULATING';await backend.updateSession(state.sessionId,{stage:next,status:SESSION_STATUS.SYNC_PENDING,measurementEndedAt:now()});await backend.appendAuditLog(state.sessionId,{eventType:'MEASUREMENT_ENDED'});if(state.session.protocolId==='EVENT_V1')await calculateResults();}
...
function surveyCompleteScreen(){
  const x=state.surveyLocked;return shell(`<div class="display-center survey-complete"><div class="complete-mark">✓</div><div class="eyebrow">RESPONSE SAVED</div><h1>回答を保存しました</h1><p class="subtitle">個別の回答内容は相手には表示されません。<br><b>PCを実験担当者へ返してください。</b></p><button class="btn btn-quiet staff-return" data-action="staff-return">スタッフ用：運用画面に戻る</button><p class="mini">${esc(x?.stage==='pre'?'測定前':'測定後')}アンケート · Participant ${esc(x?.side||'')}</p></div>`,{participant:true});
}
async function submitSurvey(form){
  ...
  try{
    ...
    const side=state.surveySide,stage=state.surveyStage;await backend.saveSurvey(state.sessionId,side,stage,data);await backend.setSurveyStatus(state.sessionId,`${stage}${side}`,true);await backend.appendAuditLog(state.sessionId,{eventType:'SURVEY_SUBMITTED',details:{side,stage}});
...
function displayScreen(){
  const s=state.session;if(s.stage==='RESULT')return resultDisplay(s);if(s.stage==='ABORTED')return phaseDisplay('MEASUREMENT ENDED','計測を終了しました。スタッフの案内をお待ちください。');
...
function explainDisplay(s){
  const pages=[
    {n:'01',title:'LOVE METER NEO について',body:'この体験では、二人で同じ質問に答えてもらい、そのときの心拍の変化を記録します。',sub:'心拍にどれくらい反応が出たか、同じ方向に変化したか、近いタイミングで反応したかなどを調べます。'},
    {n:'02',title:'おねがい',body:'質問が表示されたら、二人で自由に話してください。',sub:'正解や不正解はありません。いつも通り話して大丈夫です。'},
    {n:'03',title:'大切なこと',body:'NEO SCOREは、二人の相性や気持ちを決めるものではありません。',sub:'今回の心拍リアクションをもとにした体験用のスコアです。アンケートの回答は相手には表示されません。途中でやめたい場合は、いつでもスタッフに伝えてください。'}
  ];
...
async function calculateResults(){
  ...
  const result=computeSessionMetrics({aSamples:a,bSamples:b,timeline:s.timeline,protocolId:s.protocolId});const most=result.perQuestion.filter(x=>Number.isFinite(x.pairQResponse)).sort((x,y)=>y.pairQResponse-x.pairQResponse)[0];
...
async function handleAction(a,el){
  ...
  if(a==='start-acclim')return startAcclim();if(a==='start-measure')return startMeasure();if(a==='pause')return pauseMeasure();if(a==='resume')return resumeMeasure();if(a==='abort')return abortMeasure();if(a==='calculate')return calculateResults();
